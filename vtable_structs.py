...
class OSData(object):
	def __init__(self, os):
		self.os = os
		if os == OS_Linux:
			self.segm = idaapi.get_segm_by_name(".rodata")
		else:
			self.segm = idaapi.get_segm_by_name(".rdata")
...
def import_vtable(typename, funcs):
	...
	vstruc = idaapi.get_struc(vstrucid)

	loffs = idaapi.get_struc_last_offset(vstruc)
	if loffs != idc.BADADDR:
		idaapi.del_struc_members(vstruc, 0, loffs + 4)
	...
	try:
		...
		ti.create_ptr(ti)
		idaapi.add_struc_member(typestruc, "__vftable", 0, idc.FF_DWORD, None, 4)
		idaapi.set_member_tinfo(typestruc, idaapi.get_member(typestruc, 0), 0, ti, 0)
	except:
...
def get_os():
	return OSData(OS_Linux if idaapi.get_root_filename().endswith(".so") else OS_Win)
